...
async function getRentals(req, res) {
    await openPostgresClient(async (error) => {
        ...
        try {
            const allowedFieldsToOrder = ["id", "customerId", "daysRented", "delayFee", "gameId", "originalPrice", "rentDate", "returnDate"];
            const query = await getPostgresClient().query(queries.select("*", "rentals", customerId && !Number.isNaN(Number(customerId)) ? `"id" = ${Number.parseInt(customerId)}${status === "open" ? ` AND "returnDate" IS NULL` : status === "closed" ? ` AND "returnDate" IS NOT NULL` : ""}` : startDate && new Date(startDate) instanceof Date && !isNaN(new Date(startDate)) ? `"rentDate" >= '${startDate}'::date${status === "open" ? ` AND "returnDate" IS NULL` : status === "closed" ? ` AND "returnDate" IS NOT NULL` : ""}` : gameId && !Number.isNaN(gameId) ? `"gameId" = ${Number.parseInt(gameId)}${status === "open" ? ` AND "returnDate" IS NULL` : status === "closed" ? ` AND "returnDate" IS NOT NULL` : ""}` : !startDate && status ? status === "open" ? `"returnDate" IS NULL` : status === "closed" ? `"returnDate" IS NOT NULL` : null : null, order && allowedFieldsToOrder.includes(order) ? order : null, order && allowedFieldsToOrder.includes(order) && desc === "true" ? "DESC" : order && allowedFieldsToOrder.includes(order) ? "ASC" : null, !Number.isNaN(Number(limit)) ? Number.parseInt(limit) : null, !Number.isNaN(Number(offset)) ? Number.parseInt(offset) : null));
