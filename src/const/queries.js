const queries = {
    select: (itemsToSelect, table, whereSelectCondition, orderBy, ascOrDesc) => (`SELECT ${itemsToSelect} FROM "${table}"${whereSelectCondition ? " WHERE " + whereSelectCondition : ""}${orderBy ? ` ORDER BY "${orderBy}"` : ""}${ascOrDesc ? ` ${ascOrDesc}` : ""};`),
